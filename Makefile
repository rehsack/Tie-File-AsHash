# This Makefile is for the Tie::File::AsHash extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/Tie/File/AsHash.pm]
#     AUTHOR => q[Chris Angell <chris@chrisangell.com>]
#     NAME => q[Tie::File::AsHash]
#     PREREQ_PM => { Tie::File=>undef }
#     VERSION_FROM => q[lib/Tie/File/AsHash.pm]
...
FULLEXT = Tie/File/AsHash
BASEEXT = AsHash
PARENT_NAME = Tie::File
DLBASE = $(BASEEXT)
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Tie/File/AsHash.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DIRFILESEP)Config.pm $(PERL_INC)$(DIRFILESEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Tie/File
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Tie/File
...
TO_INST_PM = lib/Tie/File/AsHash.pm

PM_TO_BLIB = lib/Tie/File/AsHash.pm \
...
DISTNAME = Tie-File-AsHash
DISTVNAME = Tie-File-AsHash-0.05
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW)\
	  lib/Tie/File/AsHash.pm $(INST_MAN3DIR)/Tie::File::AsHash.$(MAN3EXT) 
...
realclean purge ::  clean realclean_subdirs
	...
	$(RM_RF) $(DISTVNAME)
	$(RM_F)  $(MAKEFILE_OLD) $(FIRST_MAKEFILE) blib/lib/Tie/File/AsHash.pm
...
metafile :
	...
	$(NOECHO) $(ECHO) 'name:         Tie-File-AsHash' >> META.yml
	$(NOECHO) $(ECHO) 'version:      0.05' >> META.yml
	$(NOECHO) $(ECHO) 'version_from: lib/Tie/File/AsHash.pm' >> META.yml
	$(NOECHO) $(ECHO) 'installdirs:  site' >> META.yml
	$(NOECHO) $(ECHO) 'requires:' >> META.yml
	$(NOECHO) $(ECHO) '    Tie::File:                     ' >> META.yml
...
ppd:
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Like Tie::File but access lines using a hash instead of an</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Chris Angell &lt;chris@chrisangell.com&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Tie-File" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
...
pm_to_blib: $(TO_INST_PM)
	$(NOECHO) $(PERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')'\
	  lib/Tie/File/AsHash.pm blib/lib/Tie/File/AsHash.pm 
	$(NOECHO) $(TOUCH) $@
